 ...
 * each table of the database. Currently tables must have an auto increment
 * column named "id" in order to be compatible with this class.
 *
 * @author shannah
...
public abstract class DAO<T> {
    ...
    /**
     * The tableName that this DAO works on. Each table will have its own DAO
     * object.
     */
    private final String tableName;

    /**
     * Map of column types of fields in this table. This maps field names to
     * ColTypes.
     */
    ...
    /**
     * Reference to the DAO provider for this table. This will allow subclasses
     * to load the DAO for other tables if necessary.
     */
     ...
     * data.
     *
     * @param m Map containing row data to insert. Maps column names to column
     * values.
    ...
    /**
     * Gets the generic selectById args to be used in the
    ...
    protected class Wrapper implements Observer {
        ...
        /**
         * Gets the POJO model object. If it has been GC'd, then this will
         * return null.
         *
         * @return
         */
        ...
        /**
         * Sets the POJO model object for this wrapper. This will only store a
         * weak reference so it is possible for the GC to clear the object
         * behind the scenes.
         *
    ...
    /**
     * Gets an entity object by its ID. This version only checks the cache. If
     * you want to check the database, you should use
     * {@link #getById(long,boolean)}.
     *
     * @param id The ID of the object.
     * @return The entity object with the given ID or null if it isn't already
     * loaded and cached.
     * @throws IOException
     * @deprecated {@link #getById(long id, Boolean recursive)}
     */
     ...
     *
     * @param id The ID of the object.
     * @param recursive True to retrieve child objects too. False to just load
     * the object.
     * @return The entity object with the given ID or null if it isn't already
     * loaded and cached.
     * @throws IOException
     */
    public final T getById(long id, Boolean recursive) throws IOException {
        return getById(id, false, recursive);
    }

    /**
     * Gets all of the entities in this DAO that are currently cached. Please
     * note, that the cache only stores weak references to the entities, so this
     * is not a reliable way to store a set of objects for caching and
     * retrieval.
     * <p>
     * Use {@link #fetchAll} to fetch all of the items from the data source.</p>
     *
    ...
    /**
     * Fetches a set of entities from the underlying table that match a given
     * query. This is a wrapper for {@link #fetch(java.util.Map)} that juse uses
     * a String[] array to pass the query instead of a Map.
     *
     * @param query Query parameters. Odd indices contain column names, Even
     * indices contain column values.
    ...
    /**
     * Fetches a single entity from the underlying table that matches a given
     * query.
     *
     * @param query
    ...
    /**
     * Fetchs a single entity from the underlying table that matches a given
     * query.
     *
     * @param query
     * @return An entity object or null if none was found.
     ...
     * @param refresh True to refresh from the database. False to just load from
     * cache.
     * @return
     * @throws IOException
     * @deprecated {@link #getById(long id,boolean refresh, Boolean recursive)}
     */
     ...
     * @throws IOException
     */
    public T getById(long id, boolean refresh, Boolean recursive) throws IOException {
        if (recursive == null) {
            recursive = false;
        }
        if (!refresh) {
        ...
        try {
            ...
            if (c.next()) {
                fillMap(c, m);
                T object = newObject();
                Wrapper w = initObject(id, object, m, recursive);
                return object;
    ...
    /**
     * Fetches records from the database using a specified SQL query. This is
     * protected as an encouragement for implementors to create a finite set of
     * fetchXXX methods in the subclass for performing specific searches, rather
     * than exposing SQL to the caller.
     *
    ...
    protected List<T> fetchAll(String sqlQuery, String[] params) throws IOException {
        ...
        try {
            ...
            while (c.next()) {
                ...
                if (w == null) {
                    object = newObject();
                    w = initObject((Long) m.get("id"), object, m, true);
                }
                if (w.getObject() == null) {
    ...
    /**
     * Inserts an entity into the database. This will fail if the entity is
     * already inserted.
     *
     ...
     * @see #save
     * @see #update
     * @deprecated {@link #insert(java.lang.Object, boolean) }
     */
     ...
     * @throws IOException
     * @deprecated  {@link #update(java.lang.Object, boolean) }
     */
    ...
    /**
     * Saves an entity to the database. This will check to see if the entity
     * exists already, and will call update() if not. It will call insert() if
     * it doesn't exist yet.
     *
     * @param object The entity object to save.
     * @throws IOException
     * @deprecated  {@link #save(java.lang.Object, boolean) }
     */
     ...
     * @throws IOException
     */
    public void insert(T object, boolean recursive) throws IOException {
        Log.p("Object to insert :" + object.getClass() + "/recursive:" + recursive, Log.DEBUG);
        if (recursive) {
            Map<T, ? extends DAO> mapChild = getChildDAO(object);
            if (mapChild != null) {
                for (T childObject : mapChild.keySet()) { //iterate child object
                    if (childObject != null) {
                        Log.p("childObject to insert :" + childObject.getClass(), Log.DEBUG);
                        mapChild.get(childObject).insert(childObject, true); //insert or update child object
                        mapChild.get(childObject).affectIdToParent(getId(childObject), object); //affect child object id to parent object
                    }
                }
            }
        }
     ...
     * @throws IOException
     */
    public void update(T object, boolean recursive) throws IOException {
        Log.p("Object to update :" + object.getClass() + "/recursive:" + recursive, Log.DEBUG);
        if (recursive) {
            Map<T, ? extends DAO> mapChild = getChildDAO(object);
            if (mapChild != null) {
                for (T childObject : mapChild.keySet()) { //iterate child object
                    if (childObject != null) {
                        Log.p("childObject to update :" + childObject.getClass(), Log.DEBUG);
                        mapChild.get(childObject).update(childObject, true); //insert or update child object
                        mapChild.get(childObject).affectIdToParent(getId(childObject), object); //affect child object id to parent object
                    }
                }
            }
     ...
     * @throws IOException
     */
    public void save(T object, boolean recursive) throws IOException {
        Log.p("Object to save :" + object.getClass() + "/recursive:" + recursive, Log.DEBUG);
        if (recursive) {
            Map<T, ? extends DAO> mapChild = getChildDAO(object);
            if (mapChild != null) {
                for (T childObject : mapChild.keySet()) { //iterate child object
                    if (childObject != null) {
                        Log.p("childObject to save :" + childObject.getClass(), Log.DEBUG);
                        mapChild.get(childObject).save(childObject, true); //insert or update child object
                        mapChild.get(childObject).affectIdToParent(getId(childObject), object); //affect child object id to parent object
                    }
                }
            }
    ...
    /**
     * Creates a new empty entity object for this DAO. This should be
    ...
    /**
     * Return a Map containing all child object of a parent with corresponding
     * DAO Not needed if you don't save/retrieve object recursively
     *
     * @param parentObject The parent object from which are retrieved child
     * objects
     * @return The resulting map of child object and corresponding DAO
     *
     */
    public abstract Map<T, ? extends DAO> getChildDAO(T parentObject);

    /**
     * Return the child object's id Not needed if you don't save/retrieve object
     * recursively
     *
     * @param parentObject The parent object from which are retrieved child
     * object's id
     ...
     */
    public abstract Long getChildId(T parentObject);

    /**
     * Affect the child object'id to parent, after it has been saved Not needed
     * if you don't save/retrieve object recursively
     *
     * @param id The id of childObject to affect to parentObject
     * @param parentObject The parent object from which are retrieved child
     * objects
     */
    public abstract void affectIdToParent(long id, T parentObject);

    /**
     * Affect the child object to parent, after it has been retrieve Not needed
     * if you don't save/retrieve object recursively
     *
     * @param childObject The childObject to affect to parentObject
     * @param parentObject The parent object from which are retrieved child
     * objects
     */
    public abstract void affectObjectToParent(T childObject, T parentObject);

    /**
     * Initializes a wrapper object with the given id, entity object, and
     * values.
     *
     * @param id The ID of the object.
     ...
     * @deprecated use initObject(long id, T object, Map m, boolean recursive)
     * instead
     */
     ...
     * @return The resulting wrapper.
     */
    protected Wrapper initObject(long id, T object, Map m, boolean recursive) {
        Wrapper w = new Wrapper();
        w.setObject(object);
        //w.id = id;
        if (object instanceof Observable) {
        ...
        if (recursive) {
            ...
            if (mapChild != null) {
                for (T childObject : mapChild.keySet()) { //iterate child object
                    if (childObject != null) {
                        Log.p("childObject to retrieve from BDD :" + childObject.getClass(), Log.DEBUG);
                        try {
                            T child = (T) mapChild.get(childObject).getById(mapChild.get(childObject).getChildId(object), true, true);
                            if (child != null) {
                                mapChild.get(childObject).affectObjectToParent(child, object); //affect child object id to parent object
                            }
                        } catch (Exception ex) {
                            Log.p("Cannot retrieve child object :" + childObject.getClass() + " from parent" + object.getClass(), Log.DEBUG);
                        }
    ...
    /**
     * Gets the ID of a provided entity object. Should be implemented by a
     * subclass.
     *
     ...
     *
     * @param set A Map with a nested data structure. Typically this will have
     * been parsed from JSON.
     * @param selector A selector path to indicate where, within the dataset,
     * the rows to be imported are located. E.g. "tables/data" indicates that
     * the Map set has a key "tables" with another Map, which contains a key
     * "data", which is a List of Maps - each of which represents a row to be
     * imported.
     * @param columnMap Maps columns in the imported set, into the corresponding
